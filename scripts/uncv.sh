...
{
  src=$1
  do_test "$GPAC $src -o $TEMP/dump.rgb" "decode"
  do_hash_test $src "decode"
}
...
do_test "$GPAC -i $mp4file -o $TEMP_DIR/dump_heif.rgb" "dec-heif"
...
do_test "$GPAC -i $mp4file -o $TEMP_DIR/dump_mp4.rgb" "dec-mp4"
...
do_test "$GPAC -i $mp4file -o $TEMP_DIR/dump_$suf.rgb" "dec-$suf"
