...
echo "  -re [ARG]:             removes all results (logs, stat cache and video) and run. If ARG is specified, only clean and rerun tests generated by scripts ARG."
...
echo "  -hash=SUFFIX:          regenerate tests with missing hash files. Appends '-alt-SUFFIX' to hash name for alternate hash (e.g.: for non standard platforms)"
...
do_hash_test ()
{
  ...
  if [ $# -gt 3 ] ; then
   log $L_ERR "> in test $TEST_NAME in script $current_script line $BASH_LINENO"
   log $L_ERR "	@do_hash_test takes only three arguments - wrong call (first arg is $1)"
  fi
 if [ $strict_mode = 1 ] ; then
  if [ -f $TEST_ERR_FILE ] ; then
   return
 ...
 if [ $generate_hash = 0 ] ; then
  ...
  ref_hash_suffix="$ref_hash"
  if [ -n "$generate_hash_suffix" ] ; then
    ref_hash_suffix="$ref_hash_suffix-alt-$generate_hash_suffix"
  fi

  if [ -f "$ref_hash" ] ; then

    # don't overwrite base ref if no diff (keeps mtime)
    # also don't write alt hash if it's equal to base hash
    $DIFF $test_hash $ref_hash > /dev/null
    rv=$?
    if [ $rv != 0 ] ; then
      if [ -n "$generate_hash_suffix" -a -f "$ref_hash_suffix" ] ; then
        $DIFF $test_hash $ref_hash_suffix > /dev/null
        rv=$?
        if [ $rv != 0 ] ; then
          # case 1: we're using suffix + hash exist and is different => overwrite
          cp $test_hash $ref_hash_suffix
        fi
      else
        # case 2: either not using suffix or suffix hash different => write hash file
        cp $test_hash $ref_hash_suffix
      fi
    ...
    # if no base ref create it even if we're using alt hashes
    cp $test_hash $ref_hash
    [ -n "$generate_hash_suffix" ] && cp $test_hash $ref_hash_suffix
  fi
...
do_ui_test()
{
 ...
 if [ $test_ui = 1 ] ; then
  ...
  $GPAC -runfor=$dump_dur -mp4c $src -size=$dump_size -no-save -cfg=Validator:Mode=Record -cfg=Validator:Trace=$ui_stream 2>> $ALL_LOGS
  rv=$?
 else
  if [ ! -f $ui_stream ]; then
  ...
  dur=$(($dump_dur / $speed))
  $GPAC -runfor=$dur -mp4c $src -size=$dump_size -speed=$speed -no-save -cfg=Validator:Mode=Play -cfg=Validator:Trace=$ui_stream 2>> $ALL_LOGS
  rv=$?
 fi
 #regular error, check if this is a negative test
 if [ $rv != 0 ] ; then
